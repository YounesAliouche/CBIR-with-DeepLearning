...
%         guidata(hObject, handles);
        ...
        queryImage = imresize(queryImage, [224 224]);
        ...
        %this code for ectract features of browsing image using alexnet
...
layer = 'fc7';
featuresTrain = activations(net,queryImage,layer,'OutputAs','rows');

 % construct the queryImage feature vector
        queryImageFeature = [featuresTrain str2num(name)];
        % update handles
...
elseif (metric == 1 || metric == 3 || metric == 4 || metric == 5 || metric == 6  || metric == 7 || metric == 8 || metric == 9 || metric == 10 || metric == 11)
...
for k = 1:15
    ...
    % extract query image features
    queryImage = imresize(randQueryImg, [224 224]);
    hsvHist = hsvHistogram(queryImage);
    autoCorrelogram = colorAutoCorrelogram(queryImage);
    color_moments = colorMoments(queryImage);
    % for gabor filters we need gary scale image
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[fname, pthname] = uigetfile('*.mat', 'sélectionnez l''ensemble de données (Dataset)');
if (fname ~= 0)
    ...
    if ( strcmp(lower(ext), '.mat') == 1)
        ...
        helpdlg('L''ensemble de données a été chargé avec succès !');
    else
        errordlg('Vous n''avez pas sélectionné le bon type de fichier');
    end
else
    return;
end
...
if (~isfield(handles, 'Training') && ~isfield(handles, 'numOfReturnedImages'))
    val = get(handles.popupmenuTrain, 'Value');
elseif (~isfield(handles, 'Training') || ~isfield(handles, 'numOfReturnedImages'))
    if (~isfield(handles, 'Training'))
        val = get(handles.popupmenu_DistanceFunctions, 'Value');
    else
        val = handles.Training;
    end
else
    val = handles.Training;
end
...
if val==1
    ...
    load('TrainedAlexNet.mat')
%        Charger le modéle AlexNet Entrainé  
   net = netTransfer;;
...
% --- Executes on button press in vgg19.
% % % % % function vgg19_Callback(hObject, eventdata, handles)
% % % % % % hObject    handle to vgg19 (see GCBO)
% % % % % % eventdata  reserved - to be defined in a future version of MATLAB
% % % % % % handles    structure with handles and user data (see GUIDATA)
% % % % % [query_fname, query_pathname] = uigetfile('*.jpg; *.png; *.bmp', 'Select query image');
% % % % % 
...
% % % % %     end
% % % % % 
% % % % % % --- Executes on button press in googlenet.
% % % % % function googlenet_Callback(hObject, eventdata, handles)
% % % % % % hObject    handle to googlenet (see GCBO)
% % % % % % eventdata  reserved - to be defined in a future version of MATLAB
% % % % % % handles    structure with handles and user data (see GUIDATA)
% % % % % 
% % % % % 
% % % % % % --- Executes on button press in resNet.
% % % % % function resNet_Callback(hObject, eventdata, handles)
% % % % % % hObject    handle to resNet (see GCBO)
% % % % % % eventdata  reserved - to be defined in a future version of MATLAB
% % % % % % handles    structure with handles and user data (see GUIDATA)
% % % % % [query_fname, query_pathname] = uigetfile('*.jpg; *.png; *.bmp', 'Select query image');
% % % % % 
...
% % % % %         
% % % % %        
% % % % %         %this code for ectract features of browsing image using resnet50
% % % % %         
% % % % %              if (~isfield(handles, 'Training') && ~isfield(handles, 'numOfReturnedImages'))
% % % % %     val = get(handles.popupmenuTrain, 'Value');
% % % % % elseif (~isfield(handles, 'Training') || ~isfield(handles, 'numOfReturnedImages'))
...
% % % % % 
% % % % % if val==1
% % % % %     load('TrainedResnet50.mat')
% % % % % %        load TrainedResnet50;  
% % % % %    net = TrainedResnet50;
% % % % % else
% % % % %   
% % % % %     net = resnet50;
...
% --- Executes on button press in GoogleNetCreateDataset.
function GoogleNetCreateDataset_Callback(hObject, eventdata, handles)
% hObject    handle to GoogleNetCreateDataset (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
featuresTrain = activations(net,image,layer,'OutputAs','rows');
        % add to the last column the name of image file we are processing at
        % the moment
        dataset(k, :) = [featuresTrain str2num(name)];
...
% --- Executes on button press in AlexNetCreateDataset.
function AlexNetCreateDataset_Callback(hObject, eventdata, handles)
% hObject    handle to AlexNetCreateDataset (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if (~isfield(handles, 'folder_name'))
    errordlg('Please select an image directory first!');
...
% --- Executes on button press in Vgg19CreateDataset.
function Vgg19CreateDataset_Callback(hObject, eventdata, handles)
% hObject    handle to Vgg19CreateDataset (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if (~isfield(handles, 'folder_name'))
    errordlg('Please select an image directory first!');
...
% --- Executes on selection change in popupmenuTrain.
function popupmenuTrain_Callback(hObject, eventdata, handles)
% hObject    handle to popupmenuTrain (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

handles.Training = get(handles.popupmenuTrain, 'Value');
guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function popupmenuTrain_CreateFcn(hObject, eventdata, handles)
% hObject    handle to popupmenuTrain (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in Create_dataSet.
function Create_dataSet_Callback(hObject, eventdata, handles)
% hObject    handle to Create_dataSet (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
if ( ~isempty( jpg_files ) || ~isempty( png_files ) || ~isempty( bmp_files ) )
    ...
    TypCNN = get(handles.popupmenuTrain, 'Value');
    CNN = get(handles.SelectCNN, 'Value');
...
if CNN ==1
    ...
    if TypCNN ==1
        ...
        if isfile('TrainedAlexNet.mat')
      load('TrainedAlexNet.mat')
       net = netTransfer;
        else
      warningMessage = sprintf('ATTENTION: Le CNN apres transfer Learning n''existe pas, vous devez entrainer le ConvNet D''abord ?');
...
imSize=net.Layers(1).InputSize(1);
net.Layers;
inputSize = net.Layers(1).InputSize;
...
% --- Executes on selection change in SelectCNN.
function SelectCNN_Callback(hObject, eventdata, handles)
% hObject    handle to SelectCNN (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.SCNN = get(handles.SelectCNN, 'Value');
guidata(hObject, handles);

% Hints: contents = cellstr(get(hObject,'String')) returns SelectCNN contents as cell array
%        contents{get(hObject,'Value')} returns selected item from SelectCNN


% --- Executes during object creation, after setting all properties.
function SelectCNN_CreateFcn(hObject, eventdata, handles)
% hObject    handle to SelectCNN (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in ChargerImage.
function ChargerImage_Callback(hObject, eventdata, handles)
% hObject    handle to ChargerImage (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[query_fname, query_pathname] = uigetfile('*.jpg; *.png; *.bmp', 'Sélectionnez l''image de la requête');
...
imSize=net.Layers(1).InputSize(1);
        ...
        queryImage = imresize(queryImage, [imSize imSize]);
        featuresTrain = activations(net,queryImage,layer,'OutputAs','rows');
 ...
 % construct the queryImage feature vector
        ...
        clear('query_fname', 'query_pathname', 'query_fullpath', 'pathstr', ...
            ...
            'color_moments', 'img', 'meanAmplitude', 'msEnergy', ...
            'wavelet_moments', 'queryImageFeature');
         helpdlg('L''image a été chargé avec succès !');
    else
        errordlg('Vous n''avez pas sélectionné le bon type de fichier');
    end
else
...
% --- Executes on button press in EntrainementCNN.
function EntrainementCNN_Callback(hObject, eventdata, handles)
% hObject    handle to EntrainementCNN (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
TrainCNN;
